...
def _global_evidence(analyzer: object, stats_path: Path) -> tuple[float, float | None]:
    ...
    ``0.46-309`` instead of ``0.46E-309`` in a per-mode table. PyMultiNest's
    ``get_stats`` then fails after sampling has completed. ROBERT only needs
    the global evidence lines, so it reads those directly when available.
    """
    ...
    importance = [line for line in candidates if "Importance Sampling" in line]
