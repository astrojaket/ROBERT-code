...
from .sh4 import solve_thermal_sh4, solve_thermal_sh4_spectrum
    ...
    def __post_init__(self) -> None:
        ...
        if bottom.shape != (n_spectral,):
            raise RobertValidationError(
                "bottom_contribution_radiance must match spectral grid"
            )
        ...
        if self.extinction_optical_depth is None:
            ...
            if np.any(extinction_tau < 0.0):
                raise RobertValidationError(
                    "extinction_optical_depth must be non-negative"
                )
        ...
        if geometry is None:
            ...
            if geometry.emission_angle_cosines.shape != mu.shape:
                raise RobertValidationError(
                    "geometry and emission angle quadrature must have matching shapes"
                )
            if not np.allclose(
                geometry.emission_angle_cosines, mu, rtol=1.0e-12, atol=0.0
            ):
                raise RobertValidationError(
                    "geometry emission angles must match emission_angle_cosines"
                )
            if not np.allclose(
                geometry.emission_angle_weights, weights, rtol=1.0e-12, atol=0.0
            ):
                raise RobertValidationError(
                    "geometry weights must match emission_angle_weights"
                )
        ...
        if self.point_layer_contribution_radiance is not None:
            ...
            if np.any(point_layer_contribution < 0.0):
                raise RobertValidationError(
                    "point_layer_contribution_radiance must be non-negative"
                )
        ...
        if self.point_bottom_contribution_radiance is not None:
            ...
            if np.any(point_bottom_contribution < 0.0):
                raise RobertValidationError(
                    "point_bottom_contribution_radiance must be non-negative"
                )
        ...
        if self.scattering_layer_contribution_radiance is not None:
            ...
            if np.any(scattering_layer_contribution < 0.0):
                raise RobertValidationError(
                    "scattering_layer_contribution_radiance must be non-negative"
                )
        ...
        if self.point_scattering_source_function is not None:
            ...
            if np.any(point_scattering_source < 0.0):
                raise RobertValidationError(
                    "point_scattering_source_function must be non-negative"
                )
        ...
        if self.point_scattering_contribution_radiance is not None:
            ...
            if np.any(point_scattering_contribution < 0.0):
                raise RobertValidationError(
                    "point_scattering_contribution_radiance must be non-negative"
                )

        if self.eclipse_depth is not None:
            if (
                self.eclipse_depth.spectral_grid.values.shape
                != self.radiance.spectral_grid.values.shape
            ):
                raise RobertValidationError(
                    "eclipse depth and radiance grids must have matching shapes"
                )
            if not np.allclose(
                ...
                raise RobertValidationError(
                    "eclipse depth and radiance grids must match"
                )
        ...
        object.__setattr__(
            self, "point_layer_contribution_radiance", point_layer_contribution
        )
        object.__setattr__(
            self, "point_bottom_contribution_radiance", point_bottom_contribution
        )
        object.__setattr__(
            self,
            "scattering_layer_contribution_radiance",
            scattering_layer_contribution,
        )
        object.__setattr__(
            self, "point_scattering_source_function", point_scattering_source
        )
        object.__setattr__(
            self,
            "point_scattering_contribution_radiance",
            point_scattering_contribution,
        )
...
def solve_emission(
    ...
    *,
    ...
    scattering_backend = _normalize_multiple_scattering_backend(
    ...
    if temperature_edges is not None:
        ...
        level_source_ordered = _layer_planck_source(
            wavelength, ordered_edge_temperature
        )
    (
        total_tau,
        opacity_sources,
    ...
    if scattering_source is not None:
        ...
        if not np.all(np.isfinite(stellar_mu)):
            raise RobertValidationError(
                "scattering_source requires finite geometry stellar_mu values"
            )
    ...
    if scattering_source is not None:
        point_scattering_source_ordered = np.zeros_like(
            point_layer_contribution_ordered
        )
        point_scattering_contribution_ordered = np.zeros_like(
            point_layer_contribution_ordered
        )
    bottom_source = None
    ...
    if bottom_mode == "blackbody":
        if path_geometry is None:
            ...
            bottom_visible = np.array(
                path_geometry.bottom_visible(mu), dtype=bool, copy=True
            )
    ...
    if scattering_source is None:
        ...
        for point_index, _mu_value in enumerate(mu):
            ...
            if (
                ...
                raise RobertValidationError(
                    "single-scattering source was not initialized"
                )
            mu0 = float(stellar_mu[point_index])
            if mu0 > 0.0:
                incoming_slant_tau = (
                    tau_ordered
                    * _stellar_path_factors(
                        mu0,
                        gas_optical_depth.atmosphere.n_layers,
                        order=order,
                        path_geometry=path_geometry,
                ...
                incoming_midpoint_tau = (
                    _exclusive_cumulative(incoming_slant_tau) + 0.5 * incoming_slant_tau
                )
                ...
                point_scattering_contribution_ordered[point_index] = np.sum(
                    scattering_contribution_by_g
                    * gas_optical_depth.g_weights[None, None, :],
                    axis=-1,
                )
                point_layer_contribution_ordered[point_index] += (
                    point_scattering_contribution_ordered[point_index]
                )
            if bottom_mode == "blackbody" and bottom_visible[point_index]:
                ...
                point_bottom_contribution[point_index] = (
                    np.sum(
                        total_transmission * gas_optical_depth.g_weights[None, :],
                        axis=-1,
                    )
                    * bottom_source
                )

    layer_contribution_ordered = np.tensordot(
        mu_weights, point_layer_contribution_ordered, axes=(0, 0)
    )
    bottom_contribution = np.tensordot(
        mu_weights, point_bottom_contribution, axes=(0, 0)
    )
    scattering_layer_contribution = None
    point_scattering_source = None
    point_scattering_contribution = None
    if (
        point_scattering_contribution_ordered is not None
        and point_scattering_source_ordered is not None
    ):
        ...
        scattering_layer_contribution = _restore_layer_order(
            scattering_layer_contribution_ordered, order
        )
        point_scattering_source = _restore_point_layer_order(
            point_scattering_source_ordered, order
        )
    ...
    point_layer_contribution = _restore_point_layer_order(
        point_layer_contribution_ordered, order
    )
    point_radiance = (
        np.sum(point_layer_contribution, axis=1) + point_bottom_contribution
    )
    ...
    if (
        ...
        depth = (radiance_values / stellar_radiance) * (
...
def solve_emission_spectrum(
    ...
    *,
    ...
    sh4_boundary_backend: str = "numba",
    planet_radius_m: float | None = None,
    star_radius_m: float | None = None,
    star_temperature_k: float | None = None,
    stellar_spectrum: Spectrum | None = None,
    ...
    if scattering_backend not in {"none", "sh4_henyey_greenstein_delta_m"}:
        raise RobertValidationError(
            "spectrum-only multiple scattering currently supports only 'sh4'"
        )
    ...
    scattering_applied = False
    scattering_treatment = "none"
    if scattering_backend == "sh4_henyey_greenstein_delta_m":
        (
            total_tau,
            opacity_sources,
            _scattering_treatment,
            scattering_tau,
            transport_scattering_tau,
            scattering_phase_moments,
            _scattering_sources,
        ...
        scattering_applied = bool(np.any(scattering_tau > 0.0))
        if scattering_applied:
            scattering_treatment = "rooney_p3_sh4_mixed_phase_moments_delta_m"
    else:
    ...
    if scattering_applied:
        if path_geometry is not None:
            raise RobertValidationError(
                "thermal SH4 scattering currently requires plane-parallel geometry"
            )
        if level_source_ordered is None:
            raise RobertValidationError(
                "thermal SH4 scattering requires atmosphere temperature_edges"
            )
        ...
        integrated = solve_thermal_sh4_spectrum(
            tau_ordered,
            single_scattering_albedo,
            asymmetry_factor,
    ...
    metadata = {
        ...
        "scattering_treatment": scattering_treatment,
        "multiple_scattering_backend": scattering_backend,
        "multiple_scattering_applied": str(scattering_applied).lower(),
        "thermal_integration_backend": thermal_backend,
        "opacity_sources": "+".join(opacity_sources),
        "geometry": emission_geometry.name,
        "geometry_n_points": str(emission_geometry.n_points),
        "path_geometry": "plane_parallel_secant"
...
def _resolve_emission_geometry(
    ...
    if geometry is not None:
        if emission_angle_cosines is not None or emission_angle_weights is not None:
            raise RobertValidationError(
                "geometry cannot be combined with emission angle quadrature inputs"
            )
        return geometry
...
def _validate_emission_angle_quadrature(
    ...
    if mu.shape != quadrature_weights.shape:
        raise RobertValidationError(
            "emission angle cosines and weights must have the same shape"
        )
    if np.any(mu <= 0.0) or np.any(mu > 1.0):
        raise RobertValidationError(
            "emission angle cosines must be in the interval (0, 1]"
        )
    ...
    if not np.isfinite(total_weight) or total_weight <= 0.0:
        raise RobertValidationError(
            "emission angle weights must have a finite positive sum"
        )
...
def _layer_planck_source(
    ...
    source = np.vstack(
        [
            _planck_radiance_wavelength(wavelength_micron, float(temperature))
...
def _total_optical_depth(
    ...
    if additional_optical_depths is not None:
        for contribution in additional_optical_depths:
            ...
            if _looks_like_cloud_optical_properties(contribution):
                ...
                if np.any(single_scattering_albedo < 0.0) or np.any(
                    single_scattering_albedo > 1.0
                ):
                    raise RobertValidationError(
                        "cloud single_scattering_albedo must be in [0, 1]"
                    )
                if np.any(asymmetry_factor < -1.0) or np.any(asymmetry_factor > 1.0):
                    raise RobertValidationError(
                        "cloud asymmetry_factor must be in [-1, 1]"
                    )
                ...
                if np.any(cloud_scattering_tau > 0.0):
                    ...
                    transport_scattering_tau += cloud_scattering_tau * (
                        1.0 - asymmetry_factor
                    )
                    phase_moment_numerator += (
                        cloud_scattering_tau[None, ...] * phase_moments
                    )
                continue
    ...
    if not np.all(np.isfinite(total_tau)) or np.any(total_tau < 0.0):
        raise RobertValidationError(
            "total optical depth must be finite and non-negative"
        )
    ...
    scattering_treatment = (
        "extinction_only_no_scattering_source" if has_scattering_extinction else "none"
    )
    return (
        total_tau,
        tuple(sources),
...
def _contribution_phase_moments_for_g(
    ...
    *,
    ...
    if not np.allclose(moments[0], 1.0, rtol=0.0, atol=1.0e-12):
        raise RobertValidationError(
            "scattering phase_function_moments[0] must equal one"
        )
    return moments
...
def _resolve_thermal_backend_for_solver(
    value: str, *, has_scattering_source: bool
) -> str:
...
def _layer_property_for_g(
    ...
    broadcast = np.array(
        np.broadcast_to(reshaped, target_shape), dtype=float, copy=True
    )
...
def _validate_path_geometry_match(
    ...
    if gas_edges.shape != path_edges.shape or not np.allclose(
        gas_edges, path_edges, rtol=1.0e-10, atol=0.0
    ):
        raise RobertValidationError(
            "path_geometry pressure grid must match gas optical-depth pressure grid"
        )
...
def _emission_path_factors(
    ...
    *,
    ...
    if path_geometry is None:
        ...
        factors = np.array(
            path_geometry.emission_path_factors(mu), dtype=float, copy=True
        )
    ...
    if not np.all(np.isfinite(factors)) or np.any(factors < 0.0):
        raise RobertValidationError(
            "path geometry factors must be finite and non-negative"
        )
...
def _stellar_path_factors(
    ...
    *,
    ...
    if path_geometry is None:
        ...
        factors = np.array(
            path_geometry.emission_path_factors([stellar_mu])[0], dtype=float, copy=True
        )
    if factors.shape != (n_layers,):
        raise RobertValidationError(
            "stellar path geometry factors have incorrect shape"
        )
    if not np.all(np.isfinite(factors)) or np.any(factors < 0.0):
        raise RobertValidationError(
            "stellar path geometry factors must be finite and non-negative"
        )
    ordered = factors[order]
    ordered.setflags(write=False)
    return ordered
