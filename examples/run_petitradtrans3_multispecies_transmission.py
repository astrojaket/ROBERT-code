"""Run the final stable-pRT multispecies + Rayleigh transmission reference."""
...
LINE_TO_COMPOSITION = dict(zip(LINE_SPECIES, ("H2O", "CO", "CO2", "CH4", "NH3", "HCN"), strict=True))
...
def main() -> None:
    ...
    def calculate(haze_factor: float, return_opacities: bool = False):
    ...
    metadata = {
        ...
        "timings": {
            ...
            "transmission_first_s": first_s,
            "transmission_steady_median_s": float(np.median(steady)),
        },
    }
    ...
    np.savez_compressed(
        ...
        wavelength_cm=np.asarray(rayleigh[0], dtype=float),
        transit_radius_rayleigh_cm=np.asarray(rayleigh[1], dtype=float),
        transit_radius_no_rayleigh_cm=np.asarray(clear[1], dtype=float),
