...
MISSING_PHYSICS = (
    "multi-gas random-overlap correlated-k",
    "H2-H2 and H2-He CIA",
...
def main() -> dict[str, object]:
    ...
    species = os.environ.get("ROBERT_HAT_P_32B_RT_SPECIES", DEFAULT_SPECIES).strip()
    if species != "H2O":
        raise ValueError(
            "The first ROBERT emission RT benchmark is intentionally H2O-only. "
            "Random-overlap multi-gas correlated-k is a separate RT increment."
        )
    ...
    kta_path = _species_path(kta_dir, species)
    h2o_vmr = float(os.environ.get("ROBERT_HAT_P_32B_H2O_VMR", str(DEFAULT_H2O_VMR)))
    ...
    print(f"K-table: {kta_path}")
    print(f"Species: {species}, VMR={h2o_vmr:.3e}, n_mu={n_mu}")
    ...
    composition = _composition(pressure_grid, species, h2o_vmr)
    ...
    atmosphere = AtmosphereState(
        ...
        metadata={
            ...
            "chemistry": "fixed H2O plus H2/He background",
        },
    )
...
def _composition(
    ...
    vmr = float(species_vmr)
    if not np.isfinite(vmr) or vmr < 0.0 or vmr >= 1.0:
        raise ValueError("species VMR must be finite and in [0, 1)")
    background = 1.0 - vmr
    h2 = background * DEFAULT_H2_FRACTION_OF_BACKGROUND
    he = background * (1.0 - DEFAULT_H2_FRACTION_OF_BACKGROUND)
    return {
        species: np.full(pressure_grid.n_layers, vmr),
