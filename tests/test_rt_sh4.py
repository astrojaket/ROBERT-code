...
def test_numba_is_the_default_sh4_cpu_backend() -> None:
    pytest.importorskip("numba")
    tau, omega, asymmetry, planck, mu, angle_weights, g_weights = _spectrum_case()
    ...
    assert spectrum.backend == "numba"
    assert spectrum.boundary_backend == "numba"
    assert diagnostics.boundary_backend == "numba"
