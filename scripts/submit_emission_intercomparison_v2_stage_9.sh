#!/usr/bin/env bash
# Run one production retrieval through the pinned Conda MPICH launcher.
set -euo pipefail
...
exec "$STAGE9_REPOSITORY/scripts/launch_emission_intercomparison_v2_stage_9_mpi.sh" \
  "$environment_prefix" 12 "$environment_prefix/bin/python" \
