...
The configured YAML route prepares the stellar spectra on each observation
dataset grid and applies \(\epsilon D_{\rm planet}\) to that grid's
planet-only transit-depth array inside `ParameterizedTransmissionForwardModel`.
The POSEIDON parity claim covers this dataset-grid factor semantics. It does not
establish a general high-resolution ordering relative to every instrument
response or binning operation. A separate pure-model response test covers an
explicit native-grid use case where the factor is applied before the declared
top-hat response.
