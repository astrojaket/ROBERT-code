# ROBERT Architecture Documents

Start with [RFC-0001: ROBERT Architectural Specification](../rfcs/0001-robert-architectural-specification.md).

Companion documents:

- [Software Architecture Document](software_architecture_document.md)
- [Scientific Data Model Specification](scientific_data_model_specification.md)
- [Interface Specification](interface_specification.md)
- [Repository Structure Guide](repository_structure_guide.md)
- [Developer Guidelines](developer_guidelines.md)
- [Testing & Validation Guide](testing_validation_guide.md)
- [Performance Strategy](performance_strategy.md)
- [Development Roadmap](development_roadmap.md)

User-facing status:

- [Supported Methods and Validation Status](../supported_methods.md)

Expanded reference:

- [ROBERT Software Architecture Specification v1.0](robert_software_architecture_specification_v1_0.md)
